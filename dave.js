...
const ownerNumber = ['254769995625']
...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const base64 = config.SESSION_ID.replace("", "")
  try {
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          let up = `*Hᴇʟʟᴏ ᴛʜᴇʀᴇ 『DAVE-XMD』 ᴄᴏɴɴᴇᴄᴛᴇᴅ! 👋🏻* 

*ᴋᴇᴇᴘ ᴏɴ ᴜsɪɴɢ 『DAVE-XMD』🚩* 
...
conn?.ev?.on('messages.update', async updates => {
  ...
  const udp = botNumber.split('@')[0];
    const rav = ('254769995625', '2250104610403');
