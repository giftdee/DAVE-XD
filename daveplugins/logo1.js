...
zokou({ nomCom: "hacker",
    categorie: "Dave-Logo", reaction: "👨🏿‍💻" }, async (origineMessage, zk, commandeOptions) => {
    const { prefixe, arg, ms, repondre } = commandeOptions;
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker dave");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo made by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
});
zokou({ nomCom: "dragonball", categorie: "Dave-Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
    try {
        const noArgMsg = `*_EXEMPLE *:  ${prefixe}dragonball dave`;
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
////////////////////////////
zokou({ nomCom: "naruto", categorie: "Dave-Logo", reaction: "👀" }, async (dest, zk, commandeOptions) => {
    let { ms, arg, repondre, prefixe } = commandeOptions;
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto dave");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Dave-Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t* logo by Lucky_Md*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
...
zokou({nomCom : "wall",categorie : "Dave-Logo",reaction : "👍"} , async (dest,zk,commandeOptions) =>{

  const {arg,repondre,ms,prefixe} = commandeOptions;
  if(!arg[0]) { repondre( `Exemple of using commande:\n ${prefixe}wall Dave_Md` ); return ;}
   let text = arg.join(" ")
   mumaker.textpro("https://textpro.me/break-wall-text-effect-871.html", text)
.then((data) =>{
 zk.sendMessage(dest,{image : { url : data.image},caption : 'Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃'},{quoted:ms})
}) 
.catch(console.log)
})
;
zokou({nomCom: "summer", categorie: "Dave-Logo", reaction: "🌞"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
zokou({nomCom: "neonlight", categorie: "Dave-Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  /*if (!arg[0]) {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃' }, { quoted: ms });
...
zokou({nomCom: "greenneon", categorie: "Dave-Logo", reaction: "🟢"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
zokou({nomCom: "glitch", categorie: "Dave-Logo", reaction: "🎛️"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
zokou({nomCom: "devil", categorie: "Dave-Logo", reaction: "😈"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
zokou(
  {nomCom:"boomlg",categorie:
    "Dave-Logo",reaction:"💥"},async(dest,zk,commandeOptions)=>{
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃*"})
...
//water
zokou({nomCom:"water",categorie:"Dave-Logo",reation:"💦"},async(dest,zk,commandeOptions)=>{
...
zokou({ nomCom: "transformer", categorie: "Dave-Logo", reaction: "🤖" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "thunder", categorie: "Dave-Logo", reaction: "⚡" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "harrypotter", categorie: "Dave-Logo", reaction: "🧙‍♂️" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "cat", categorie: "Dave-Logo", reaction: "🪟" }, async (dest, zk, commandeOptions) => {
...
zokou({ nomCom: "whitegold", categorie: "Dave-Logo", reaction: "💫" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "lightglow", categorie: "Dave-Logo", reaction: "🌟" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "thor", categorie: "Dave-Logo", reaction: "🔨" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({ nomCom: "neon", categorie: "Dave-Logo", reaction: "💡" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
zokou({nomCom:"purple",categorie:"Dave-Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
  var {ms,repondre,prefixe,arg}=commandeOptions;

    try{
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple dave");return;}
...
zokou({nomCom:"gold",categorie:"Dave-Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃");return;
      }
...
zokou({nomCom:"arena",categorie:"Dave-Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Dave-Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃");return;
      }
...
zokou({ nomCom: "gif×1", categorie: "Dave-Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  let { ms, arg, prefixe, repondre } = commandeOptions;
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃`);
      return;
    }
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*Logo by 𝐃𝐀𝐕𝐄-𝐗𝐌𝐃 *" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
